...
export default async function handler(req,res){
  ...
  try{
    ...
    const subject=`Nueva consulta AxerTech - reparaciondeportatilesmadrid.com.es | ${modelo||'Portátil Acer'}`;
    const html=`<div style="font-family:Arial,sans-serif;line-height:1.6;color:#141C24"><h2 style="color:#0F4C90">Nueva consulta desde AxerTech</h2><p><strong>Nombre:</strong> ${safe.nombre}</p><p><strong>Teléfono:</strong> ${safe.telefono}</p><p><strong>Email:</strong> ${safe.email}</p><p><strong>Modelo Acer:</strong> ${safe.modelo}</p><p><strong>Descripción de la avería:</strong><br>${safe.mensaje}</p><hr><p style="font-size:12px;color:#57646F">Enviado desde el formulario de reparaciondeportatilesmadrid.com.es</p></div>`;
    const mime=[`From: AxerTech <${from}>`,`To: ${to}`,`Reply-To: ${email}`,`Subject: ${encodeHeader(subject)}`,'MIME-Version: 1.0','Content-Type: text/html; charset="UTF-8"','Content-Transfer-Encoding: 8bit','',html].join('\r\n');
